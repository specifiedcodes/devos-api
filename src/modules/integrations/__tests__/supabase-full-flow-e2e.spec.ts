...
/**
 * Supabase Full Lifecycle E2E Integration Test
 * Story 15-6: AC11 - Complete lifecycle flow
 *
 ...
 */
describe('Supabase E2E - Full Lifecycle Flow', () => {
  ...
  describe('AC11: Full Supabase OAuth Lifecycle', () => {
    it('should complete full Supabase OAuth lifecycle: authorize -> callback -> status -> disconnect -> re-authorize', async () => {
      // ========== Step 1: Generate Supabase authorization URL ==========
      const authResult = await service.generateSupabaseAuthorizationUrl(
        MOCK_USER_ID,
        MOCK_WORKSPACE_ID,
      );
      ...
      const stateKey = `supabase-oauth-state:${state}`;
      ...
      // ========== Step 4: Simulate Supabase callback ==========
      mockHttpService.post.mockReturnValueOnce(
        of(createAxiosResponse(MOCK_SUPABASE_TOKEN_RESPONSE)),
      );

      const callbackResult = await service.handleSupabaseCallback(
        'valid-auth-code',
        state!,
      );
      expect(callbackResult.redirectUrl).toContain('supabase=connected');

      // ========== Step 5: Verify integration record created in database ==========
      const dbRecord = dbStore.get(MOCK_INTEGRATION_ID);
      expect(dbRecord).toBeDefined();
      expect(dbRecord.provider).toBe(IntegrationProvider.SUPABASE);
      expect(dbRecord.status).toBe(IntegrationStatus.ACTIVE);

      // ========== Step 6: Verify token is encrypted (not plaintext) ==========
      expect(dbRecord.encryptedAccessToken).not.toBe(MOCK_SUPABASE_TOKEN);
      ...
      expect(decryptedToken).toBe(MOCK_SUPABASE_TOKEN);

      // ========== Step 8: Check Supabase status returns connected: true ==========
      const statusResult = await service.getSupabaseStatus(MOCK_WORKSPACE_ID);
      expect(statusResult.connected).toBe(true);

      // ========== Step 9: List integrations and verify Supabase appears ==========
      const integrationsResult = await service.getIntegrations(
        MOCK_WORKSPACE_ID,
      );
      expect(integrationsResult.length).toBeGreaterThanOrEqual(1);
      const supabaseIntegration = integrationsResult.find(
        (i) => i.provider === 'supabase',
      );
      ...
      const statusAfterDisconnect = await service.getSupabaseStatus(
        MOCK_WORKSPACE_ID,
      );
      ...
      const reAuthResult = await service.generateSupabaseAuthorizationUrl(
        MOCK_USER_ID,
        MOCK_WORKSPACE_ID,
      );
      ...
      const newCallbackResult = await service.handleSupabaseCallback(
        'new-auth-code',
        newState!,
      );
      expect(newCallbackResult.redirectUrl).toContain('supabase=connected');
