...
/**
 * Railway Full Lifecycle E2E Integration Test
 * Story 15-4: AC11 - Complete lifecycle flow
 *
 ...
 */
describe('Railway E2E - Full Lifecycle Flow', () => {
  ...
  describe('AC11: Full Railway OAuth Lifecycle', () => {
    it('should complete full Railway OAuth lifecycle: authorize -> callback -> status -> disconnect -> re-authorize', async () => {
      // ========== Step 1: Generate Railway authorization URL ==========
      ...
      const authResult = await service.generateRailwayAuthorizationUrl(
        MOCK_USER_ID,
        MOCK_WORKSPACE_ID,
      );

      expect(authResult.authorizationUrl).toContain('https://railway.app/authorize');
      ...
      const stateKey = `railway-oauth-state:${state}`;
      ...
      // ========== Step 4: Simulate Railway callback ==========
      const callbackResult = await service.handleRailwayCallback(
        'valid-auth-code',
        state!,
      );
      expect(callbackResult.redirectUrl).toContain('railway=connected');

      // ========== Step 5: Verify integration record created in database ==========
      const dbRecord = dbStore.get(MOCK_INTEGRATION_ID);
      expect(dbRecord).toBeDefined();
      expect(dbRecord.provider).toBe(IntegrationProvider.RAILWAY);
      expect(dbRecord.status).toBe(IntegrationStatus.ACTIVE);
      expect(dbRecord.externalUsername).toBe(MOCK_RAILWAY_USER.name);

      // ========== Step 6: Verify token is encrypted (not plaintext) ==========
      expect(dbRecord.encryptedAccessToken).not.toBe(MOCK_RAILWAY_TOKEN);
      ...
      expect(decryptedToken).toBe(MOCK_RAILWAY_TOKEN);

      // ========== Step 8: Check Railway status returns connected: true ==========
      const statusResult = await service.getRailwayStatus(MOCK_WORKSPACE_ID);
      expect(statusResult.connected).toBe(true);
      expect(statusResult.username).toBe(MOCK_RAILWAY_USER.name);

      // ========== Step 9: List integrations and verify Railway appears ==========
      const integrationsResult = await service.getIntegrations(MOCK_WORKSPACE_ID);
      expect(integrationsResult.length).toBeGreaterThanOrEqual(1);
      const railwayIntegration = integrationsResult.find(
        (i) => i.provider === 'railway',
      );
      ...
      const statusAfterDisconnect = await service.getRailwayStatus(
        MOCK_WORKSPACE_ID,
      );
      ...
      const reAuthResult = await service.generateRailwayAuthorizationUrl(
        MOCK_USER_ID,
        MOCK_WORKSPACE_ID,
      );
      ...
      const newCallbackResult = await service.handleRailwayCallback(
        'new-auth-code',
        newState!,
      );
      expect(newCallbackResult.redirectUrl).toContain('railway=connected');
